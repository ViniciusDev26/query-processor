 ...
 */
function applyProjectionPushdown(
...
function optimizeProjection(projection: Projection): RelationalAlgebraNode {
	const normalizedAttributes = normalizeProjectionAttributes(
	...
	if (optimizedInput.type === "Projection") {
		const innerAttributes = normalizeProjectionAttributes(
			optimizedInput.attributes,
		);
		if (
			innerAttributes.includes("*") ||
			normalizedAttributes.every((attr) =>
				projectionHasAttribute(innerAttributes, attr),
			)
		) {
 ...
 */
function optimizeSelectionForProjection(
	selection: Selection,
): RelationalAlgebraNode {
	const optimizedInput = applyProjectionPushdown(selection.input);
	const requiredAttributes = extractAttributesFromCondition(
		selection.condition,
	);
	...
	if (
		...
		const ensured = ensureProjectionAttributes(
			optimizedInput.attributes,
			requiredAttributes,
		);
...
function projectionHasAttribute(
	attributes: string[],
	attribute: string,
): boolean {
	if (attributes.includes("*")) {
...
function _canPushProjectionThroughSelection(
	attributes: string[],
	condition: string,
): boolean {
	if (attributes.includes("*")) {
	...
	return requiredAttributes.every((attr) =>
		attributeSetHas(attributeSet, attr),
	);
}
...
function ensureProjectionAttributes(
	...
	if (requiredAttributes.length === 0) {
		return {
			attributes: normalizeProjectionAttributes(attributes),
			changed: false,
		};
	}
 ...
 */
function optimizeCrossProductForProjection(
	node: CrossProduct,
): RelationalAlgebraNode {
	return {
		type: "CrossProduct",
...
export const projectionPushdownRule: OptimizationRuleMetadata = {
	...
	description:
		"Pushes projection operations closer to base relations to reduce attribute count early",
	category: "heuristic",
